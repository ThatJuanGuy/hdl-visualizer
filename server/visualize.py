...
def drawHDL(fileContent):
    ############ global vars #############################
    id2part = {}
    op2expr = {}
    hdl=parse_hdl(fileContent) # instead of reading a file now you just pass the entire
                               # file contents.
    ...
    fig = elements4output(logician(outs, ins))
    figBytes = fig.get_imagedata(ImageFormat.SVG)
    return figBytes
